...
fs.writeFileSync(path.join(jsDir, "store-manual-checkout.js"), `
(function(){
...
const checkoutFiles = ["checkout.html", "cart.html", "thank-you.html"];

for (const name of checkoutFiles) {
  const file = path.join(publicDir, name);
  if (!fs.existsSync(file)) continue;

  let html = fs.readFileSync(file, "utf8");

  html = html.replace(/<script[^>]+store-manual-checkout\.js[^>]*><\/script>\s*/gi, "");

  // Load early so it catches checkout fetch before page scripts run
  html = html.replace("</head>", '  <script src="/js/store-manual-checkout.js"></script>\n</head>');

  fs.writeFileSync(file, html);
  console.log("Manual checkout controller added to:", name);
}

console.log("Store Yoco redirect paused. Manual checkout enabled.");
