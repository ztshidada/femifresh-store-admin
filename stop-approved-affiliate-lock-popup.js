...
(async function(){
  ...
  function pageAlreadyLooksApproved(){
  ...
  async function getLiveAffiliate(){
    ...
    for (const url of urls) {
      try {
        ...
        if (a && typeof a === "object" && (a.email || a.id || a.referralCode)) {
          found = a;
          break;
        }
      } catch(e) {}
    ...
    if (!found && (local.email || local.id || local.referralCode)) {
      found = local;
    }
  ...
  function updateGreeting(a){
    ...
    document.querySelectorAll("*").forEach(el => {
      ...
      if (text.includes("Hi Affiliate")) {
        el.textContent = text.replace("Hi Affiliate", "Hi " + name);
      }
    });
  }
  ...
  async function boot(){
    ...
    // Give the normal dashboard time to load first.
    setTimeout(async () => {
      if (pageAlreadyLooksApproved()) {
        return;
      }

      const a = await getLiveAffiliate();

      // Important: if we cannot confirm the affiliate, do not lock the dashboard.
      if (!a) {
        return;
      }
      ...
      // Last safety check before locking.
      if (pageAlreadyLooksApproved()) {
        return;
      }
